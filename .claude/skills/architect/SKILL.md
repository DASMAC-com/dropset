...
**Check the model before doing anything else**, the same guard
`plan` carries: this skill is Fable/Mythos-mandated, and a
session that started on the implementation default should stop
and say so rather than doing architecture on the cheap tier.
