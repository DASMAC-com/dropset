...
as `{ file, line, dimension, severity
(high/med/low), fingerprint_slug, title, rationale,
fix_sketch }`. Dimensions:
...
- Otherwise query live Linear —
  `mcp__claude_ai_Linear__list_issues(team: "84659a7c-5ea3-47b1-b2bd-c531e3721d6b", project: "d505fe50-cc8b-41ca-be93-6215d9adcea0", query: "<basename + topic>", limit: 50)`
  — and if an unresolved audit-loop subtask (parent
  ENG-452, in Backlog) already covers the same
  file + topic, record its id into `findings.json`
  and **skip filing**.
...
- `**Discovered by**: platform-audit-loop iteration
  <n> @ <git sha>` so it reproduces against the
  exact revision.

After each `save_issue`, append
...
```txt
iteration <n> | mode <file|pr|survey> | subject <…> | filed <k> (h/m/l) | deduped <d> | skip.txt += <globs>
```
