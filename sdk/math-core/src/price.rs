...
//!
//! This is the **canonical** codec: the on-chain program depends on this
//! crate for `Price`, and the off-chain SDK / WASM client share the same
//! code, so the live engine and any router quoting off the simulator can
//! no longer drift. The TS `price.ts` is the one remaining intentional
//! cross-language re-impl, pinned to this code by the conformance vectors.
//!
//! # Validation
//!
//! `Price` derives `Pod`, so deserialization will turn arbitrary bytes
...
const SIGNIFICAND_MASK: u32 = (1u32 << SIGNIFICAND_BITS) - 1;
...
const UNBIASED_EXPONENT_MIN: i8 = 0 - BIAS as i8;

/// Largest unbiased exponent (`15`).
const UNBIASED_EXPONENT_MAX: i8 = MAX_BIASED_EXPONENT as i8 - BIAS as i8;
...
#[cfg_attr(feature = "idl", derive(anchor_lang_v2::IdlType))]
...
impl Price {
    ...
    /// trusted on-chain storage where the bits were written by the
    /// program.
    ///
    ...
    /// math (it is also the basis of the WASM `price_decode` binding).
...
mod tests {
    ...
    #[test]
    fn from_value_fx() {
        let p = Price::from_value(1.085).unwrap();
        assert!((p.to_f64() - 1.085).abs() < 1e-7);
    }

    #[test]
    fn from_value_zero_and_invalid() {
        assert_eq!(Price::from_value(0.0), Some(Price::ZERO));
