...
//!
//! ## Why the walk must finish before anything is emitted
//!
//! The tempting bounded fix — walk back a few pages, emit the oldest one
//! reached, advance, repeat — is wrong, and wrong in the same direction as
//! the bug it is meant to fix. The resume cursor is an *exclusive lower
//! bound* (`until`): moving it to a mid-backlog position tells the next
//! request to ignore everything older, so any page below the one just
//! emitted is discarded rather than deferred.
//!
//! The oldest unemitted record is therefore only identifiable once the walk
//! has reached the resume bound. This pager enforces that: it hands back
//! pages to emit only after the walk reports it got there, and until then it
//! asks for more enumeration.
//!
//! ## What a page key is
//!
//! The pager stores a **page key**, not a page. For an RPC source that key
//! is the `before` marker the page was fetched with — one signature per
//! page rather than one per record, which is what keeps a deep backlog's
//! bookkeeping small enough that the walk can afford to run to the bound.
...
impl<K> Backfill<K> {
    ...
    pub fn step(&mut self) -> Step<K> {
        if !self.reached_bound {
            return Step::Enumerate;
        }
        match self.pending.pop() {
            Some(key) => Step::Emit(key),
            None => Step::Done,
        }
    }
...
mod tests {
    ...
    fn withholds_every_page_until_the_walk_reaches_the_bound() {
        ...
        assert_eq!(pager.step(), Step::Enumerate);
        ...
        assert_eq!(pager.step(), Step::Emit("oldest"));
        assert_eq!(pager.step(), Step::Emit("middle"));
        assert_eq!(pager.step(), Step::Emit("newest"));
        assert_eq!(pager.step(), Step::Done);
    }
    ...
    fn restart_returns_the_pager_to_enumerating() {
        ...
        assert_eq!(pager.step(), Step::Emit("only"));
        assert!(pager.caught_up());
