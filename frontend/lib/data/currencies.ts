...
// Localnet on-chain mint overlay: symbol → { mint, tokenProgram } for the mock
// demo mints the TUI bootstrap creates (tui/src/market.rs, from the checked-in
// keys/*.json). NOT applied to the display data — every stablecoin keeps its
// real mainnet mint so prices, metadata, and the currencies page render real
// market data on any cluster. The mock mints are substituted only at the
// on-chain boundary, via onchainMint / onchainTokenProgram below.
import localnetMints from "./currencies.localnet.json";
// Generated by scripts/build-token-manifest.mjs from the icons committed
// under brand-assets/token-icons/: symbol → "/token-icons/<file>". Built
// from committed bytes with no network access, so this mapping can no longer
// be thinned by a third-party host being down at build time — the case the
// fallback below was originally load-bearing for. The fallback is NOT
// vestigial, though: a symbol with no committed icon, or with two (which is
// what an upstream format change leaves behind), is deliberately omitted from
// the manifest and resolves through the remote URL instead.
// Consulted as a lookup rather than merged into the imported data, so the
// declared upstream URL in currencies.json always survives as a fallback.
// Overwriting `s.icon` in place used to destroy it, which left a symbol
// whose mirrored file was missing or corrupt with no recovery path at all —
// the browser rendered nothing, silently. See tokenIconUrl below.
...
// The mock demo mint (and its token program) for a real mainnet mint, when
// running on localnet. Everything above keeps the real mainnet mint so display
// data (prices, metadata) is real on any cluster; these two helpers do the
// last-moment substitution at the on-chain boundary — ATA derivation, market
// PDA resolution, and instruction building — so balances, quotes, and swaps
// hit the actual local accounts. Both are the identity on mainnet.
type OnchainOverride = { mint: string; tokenProgram: TokenProgramKind };
const LOCALNET_BY_SYMBOL = localnetMints as Record<string, OnchainOverride>;

export const onchainMint = (mint: string): string => {
  if (!IS_LOCALNET) return mint;
  const symbol = STABLE_BY_MINT[mint]?.symbol;
  return (symbol ? LOCALNET_BY_SYMBOL[symbol]?.mint : undefined) ?? mint;
};

export const onchainTokenProgram = (mint: string): TokenProgramKind => {
  const real = STABLE_BY_MINT[mint]?.tokenProgram ?? "classic";
  if (!IS_LOCALNET) return real;
  const symbol = STABLE_BY_MINT[mint]?.symbol;
  return (
    (symbol ? LOCALNET_BY_SYMBOL[symbol]?.tokenProgram : undefined) ?? real
  );
};
...
// back to the issuer's canonical URL when the symbol is absent from the
// manifest (a token added since the last build, or one whose fetch failed).
export const tokenIconUrl = (symbol: string): string =>
  ICON_OVERRIDES[symbol] ?? STABLE_BY_SYMBOL[symbol]?.icon ?? "";

// The canonical issuer URL, kept reachable so a render-side error handler can
// retry off-origin when the mirrored file is missing or unreadable. Returns
// "" when it is already what tokenIconUrl produced, so a caller can tell
// "nothing further to try" from "try this next" without comparing strings.
export const tokenIconFallbackUrl = (symbol: string): string => {
  const canonical = STABLE_BY_SYMBOL[symbol]?.icon ?? "";
  return ICON_OVERRIDES[symbol] ? canonical : "";
};
