...
across many vaults, so a fill is emitted **per matched `(vault, level)`** plus
one **take-level summary**.
...
| `vault` | the matched vault |
| `leader` | the vault's economic owner |
...
take envelope); only a zero-fill-below-minimum reverts. Each router needs a
**CU / account-count budget** for a full active-vault-set take, **including the
+2 `event-cpi` accounts** the fill appends (see **architecture.md → Events and
emission**); consider a bounded top-of-book fast path if a full reconstruction
is too heavy, and validate each router's actual swap-CPI contract before
assuming a shared trait.
