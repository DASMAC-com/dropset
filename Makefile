...
collectors-up: check-docker
	docker compose -f infra/localnet/docker-compose.yml \
		up -d --build --quiet-pull postgres migrate $(KEYLESS_SERVICES) grafana
	@$(KEYED_UP)
...
# `fair_price`. It
# belongs in this variable anyway, and the reason is what the variable is for
# rather than what its members have in common — it is keyless, and both the
# bring-up and the teardown have to name it or its container survives into the
# next bring-up. Splitting it into a second variable would buy an accurate name
# and reintroduce exactly the lose-a-name failure the single list prevents.
