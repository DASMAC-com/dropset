...
async fn main() -> anyhow::Result<()> {
    ...
    // The request is keyed by base currency in its path (`/v6/latest/USD`), so
    // every pair this venue can serve quotes against USD — see
    // `usd_quoted_currencies`, which the sibling Frankfurter collector shares.
    let UsdRoster {
        by_currency,
        currencies,
    } = usd_quoted_currencies("er-api", &ids)?;
