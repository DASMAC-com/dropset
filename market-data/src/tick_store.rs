...
/// framework's spawn / backoff machinery can have it there.
///
/// A **publisher** consuming this should call [`Self::latest`] directly instead.
/// The framework runner sleeps its backoff and continues on any source error,
/// forever — right for a collector, where a missed poll is a gap in a series, and
/// wrong for something that has to decide whether a failure is worth retrying at
/// all. The fair-value estimator is the first such consumer and takes that path.
