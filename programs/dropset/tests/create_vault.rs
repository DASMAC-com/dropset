...
#[test]
fn admin_can_create_vault_for_another_leader() {
    ...
    f.create_vault(100_000, foreign.pubkey(), false, foreign.pubkey())
...
fn non_admin_with_foreign_leader_override_rejects() {
    ...
    // `create_vault_as` pre-funds so the rejection isn't masked by an
    // insufficient-funds error.
    let err = f
        .create_vault_as(&outsider, 0, outsider.pubkey(), false, foreign.pubkey())
...
fn rejects_default_quote_authority() {
    let mut f = Fixture::bootstrap();
    let err = f
        .create_vault(0, Pubkey::default(), false, Pubkey::default())
