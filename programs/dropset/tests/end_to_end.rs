...
/// Full outside-depositor lifecycle on the shared `Fixture`:
/// seed → admin approves → leader opts in → outside `deposit` →
/// partial `withdraw` → full `withdraw` (PDA close + counter
/// decrement). Gates on both WI1b setters shipping.
#[test]
